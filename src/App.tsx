...
import { 
  ...
  fetchSettingsFromSupabaseDirect,
  fetchParticipantsCountFromSupabase 
} from './lib/supabase';
...
export default function App() {
  ...
  // Throttle timestamp ref to strictly avoid thundering herds and redundant calls
  const lastFetchTimeRef = useRef<number>(0);
  const THROTTLE_INTERVAL_MS = 45000; // Minimum 45s between global fetches

  const fetchGlobalCounterAndSettings = async (force = false) => {
    const now = Date.now();
    if (!force && now - lastFetchTimeRef.current < THROTTLE_INTERVAL_MS) {
      return;
    }
    ...
    try {
      const statsPromise = fetch('/api/stats').then(async (res) => {
        ...
        // Zero-Egress count query using HEAD request (0 bytes of data egress!)
        try {
          const count = await fetchParticipantsCountFromSupabase();
      ...
      const settingsPromise = fetch('/api/settings/public').then(async (res) => {
        if (res.ok) {
          const contentType = res.headers.get('content-type') || '';
          if (contentType.includes('application/json')) {
  ...
  useEffect(() => {
    fetchGlobalCounterAndSettings(true);

    // Periodic polling every 90 seconds (reduced frequency to conserve free tier bandwidth)
    const intervalId = setInterval(() => {
      if (document.visibilityState === 'visible') {
        fetchGlobalCounterAndSettings(false);
      }
    }, 90000);
    ...
    window.addEventListener('ccb_participant_registered', () => fetchGlobalCounterAndSettings(true));

    return () => {
      clearInterval(intervalId);
      window.removeEventListener('focus', handleFocusOrVisible);
      document.removeEventListener('visibilitychange', handleFocusOrVisible);
      window.removeEventListener('ccb_participant_registered', () => fetchGlobalCounterAndSettings(true));
    };
  }, []);

  // Check on tab switch only if stale
  useEffect(() => {
    fetchGlobalCounterAndSettings(false);
  }, [activeTab]);
