...
export function formatAudioUrl(rawUrl: string): string {
  ...
  // If already base64, blob URL, or local upload path
  if (trimmed.startsWith('data:') || trimmed.startsWith('blob:') || trimmed.startsWith('/uploads/') || trimmed.startsWith('/api/audio-proxy') || trimmed.startsWith('/audio-proxy')) {
    return trimmed;
  }

  // Google Drive: extract file ID and stream through our resilient audio proxy endpoint
  const gDriveMatch = trimmed.match(/drive\.google\.com\/(?:file\/d\/|open\?id=|uc\?(?:export=download&)?id=)([a-zA-Z0-9_-]+)/);
  if (gDriveMatch && gDriveMatch[1]) {
    const fileId = gDriveMatch[1];
