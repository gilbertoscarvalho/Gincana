...
export const exportParticipantsToExcel = (
  ...
  const mainData = participants.map((p, idx) => {
    ...
    return {
      ...
      'Valor Contribuído (R$)': (p.amountPaid !== undefined && p.amountPaid !== null ? Number(p.amountPaid) : 20).toFixed(2),
...
export const exportParticipantsToPDF = (
  ...
  autoTable(doc, {
    ...
    columnStyles: {
      ...
      2: { cellWidth: 90 },                   // Congregação
      3: { cellWidth: 28, halign: 'center' }, // Idade
      4: { cellWidth: 38, halign: 'center' }, // Gincana
      5: { cellWidth: 38, halign: 'center' }, // Tocata
      6: { cellWidth: 75 },                   // Instrumento
      7: { cellWidth: 90 },                   // Alimento
      8: { cellWidth: 55, halign: 'right' },  // Valor
      9: { cellWidth: 70, halign: 'center' }, // Comprovante
