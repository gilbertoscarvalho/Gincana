...
export function requestGoogleDriveToken(onSuccess: (token: string) => void, onError: (err: any) => void) {
  if (!window.google || !window.google.accounts || !window.google.accounts.oauth2) {
    onError(new Error('Google Identity Services library not loaded yet. Please refresh the page.'));
    return;
  }

  try {
    const tokenClient = window.google.accounts.oauth2.initTokenClient({
      client_id: '', // Will use standard OAuth consent window if configured, or implicit flow
