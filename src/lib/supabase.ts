...
export const DEFAULT_SUPABASE_ANON_KEY = typeof Buffer !== 'undefined'
  ...
  : (typeof atob !== 'undefined' ? atob('') : '');
...
export const DEFAULT_SUPABASE_KEY = DEFAULT_SUPABASE_ANON_KEY || DEFAULT_SUPABASE_PUBLISHABLE_KEY;
...
export function getSupabaseCredentials(): { url: string; key: string } {
  ...
  if (!url && metaEnv) {
    const rawUrl = (metaEnv.VITE_SUPABASE_URL || metaEnv.NEXT_PUBLIC_SUPABASE_URL || metaEnv.SUPABASE_URL || '').trim();
    if (!isSuspendedOrInvalidSupabaseUrl(rawUrl)) {
      url = rawUrl;
    }
  }
...
// In the browser, dynamically check if server provides live Supabase config (e.g. from Vercel env vars)
if (typeof window !== 'undefined') {
  fetch('/api/supabase/config')
    .then((res) => (res.ok ? res.json() : null))
    .then((data) => {
      if (data && data.url && data.key) {
        setCustomSupabaseCredentials(data.url, data.key);
      }
    })
    .catch(() => {});
}
...
// Helper to execute upsert with schema tolerance (prioritizing PostgreSQL exact snake_case schema)
async function executeResilientSupabaseUpsert(supabase: any, row: Record<string, any>): Promise<boolean> {
  // Primary: Strict snake_case row (the exact schema of the Supabase PostgreSQL table)
  ...
  // Attempt 3: If already exists (duplicate key error), perform Update
  ...
  console.warn('[Supabase Direct] Notice upserting participant with all schema fallbacks:', snakeErr?.message || insertErr?.message || baseSnakeErr?.message);
  return false;
}
...
export async function getSupabaseLiveHealth(customUrl?: string, customKey?: string): Promise<{
  ...
  try {
    ...
    return {
      connected: true,
      participantCount: res.count !== null && res.count !== undefined ? res.count : (res.data?.length || 0),
      projectUrl: cleanUrl
    };
  } catch (err: any) {
    return {
