...
export const DEFAULT_SUPABASE_PUBLISHABLE_KEY = 'sb_publishable_a_qUOnQnE8Vi9i5y0JsE0w_-lTNbc3C';
...
export function isSuspendedOrInvalidSupabaseKey(k?: string | null): boolean {
  ...
  if (trimmed.includes('tcmurarerhzrcgfqanhf')) return true;
  if (trimmed.includes('bFHLvYG2nK8qA6so')) return true;
  try {
  ...
  try {
    ...
    if (success) {
      if (clientDirectCache) {
        ...
        setPersistentParticipantCache(updated);
      } else {
        clientDirectCache = { data: [p], timestamp: Date.now() };
        setPersistentParticipantCache([p]);
      }
    }
    return success;
  } catch (err) {
...
export async function deleteParticipantFromSupabaseDirect(id: string): Promise<boolean> {
  ...
  try {
    ...
    if (!error) {
      if (clientDirectCache) {
        const filtered = clientDirectCache.data.filter((item) => item.id !== id);
        clientDirectCache = { data: filtered, timestamp: Date.now() };
        setPersistentParticipantCache(filtered);
      }
      return true;
    }
    return false;
