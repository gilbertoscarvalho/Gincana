...
export const DEFAULT_SUPABASE_JWKS_URL = 'https://ykhzsdnbprosatvpygbb.supabase.co/auth/v1/.well-known/jwks.json';
export const DEFAULT_SUPABASE_KEY = 'sb_publishable_a_qUOnQnE8Vi9i5y0JsE0w_-lTNbc3C';
...
export function getStoredCustomCredentials(): { url: string; key: string } {
  if (typeof window !== 'undefined' && window.localStorage) {
    ...
    // Ignore stale keys from the legacy project
    if (key.includes('t0V20lTkdh')) {
...
export function getSupabaseCredentials(): { url: string; key: string } {
  ...
  if (typeof process !== 'undefined' && process.env) {
    ...
    // Discard stale credentials pointing to the old project
    const validEnvKeys = envKeys.filter(k => !k.includes('t0V20lTkdh'));
    key = validEnvKeys[0] || '';
  }
  ...
  if (!key && metaEnv) {
    ...
    const validClientKeys = clientKeys.filter(k => !k.includes('t0V20lTkdh'));
    key = validClientKeys[0] || '';
  }

  // 4. Guaranteed project fallback for Vercel/GitHub deployments
  if (!url) url = DEFAULT_SUPABASE_URL;
  if (!key || key.includes('t0V20lTkdh')) key = DEFAULT_SUPABASE_KEY;
