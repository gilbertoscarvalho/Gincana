    ...
    try {
      ...
      try {
        ...
        if (res.ok) {
          ...
          // If server returned a business error message (like missing field), parse it
          const errText = await res.text();
          try {
            const parsedErr = JSON.parse(errText);
            if (parsedErr.error && res.status === 400) {
        ...
        if (networkErr.message && !networkErr.message.includes('comunicação') && !networkErr.message.includes('500') && !networkErr.message.includes('Failed to fetch')) {
          throw networkErr;
        }
        console.warn('Network / server error on submit, saving locally as backup:', networkErr);
      }

      // If server could not be reached, generate valid local participant registration
      if (!participantData) {
        const nameParts = cleanName.split(' ').filter(Boolean);
        participantData = {
          id: 'p-' + Date.now() + '-' + Math.floor(Math.random() * 10000),
          fullName: cleanName,
          firstName: nameParts[0] ? nameParts[0].toLowerCase() : cleanName.toLowerCase(),
          congregation: finalCongregation,
          age: Number(age),
          foodOrDrink: foodOrDrink.trim(),
          activities: {
          ...
          proofFileName: proofFile ? proofFile.name : null,
          proofFileType: proofFile ? proofFile.type : null,
          proofStatus: proofFile ? 'Analisando' : 'Pendente',
          createdAt: new Date().toISOString(),
          updatedAt: new Date().toISOString()
        };
      }

      // Save to browser localStorage as persistent backup
      try {
        const existingLocal = JSON.parse(localStorage.getItem('ccb_gincana_local_participants') || '[]');
        const updatedLocal = [participantData, ...existingLocal.filter((p: any) => p.id !== participantData.id)];
        localStorage.setItem('ccb_gincana_local_participants', JSON.stringify(updatedLocal));
      } catch (storageErr) {
        console.warn('Local storage backup error:', storageErr);
      }
