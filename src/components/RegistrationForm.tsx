...
export const RegistrationForm: React.FC<RegistrationFormProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Execute dual-channel persistence (Server API + Direct Supabase) in parallel
      const payloadWithId = { ...payload, id: participantId };

      const serverPromise = fetch('/api/participants', {
        method: 'POST',
        headers: {
      ...
      const supabasePromise = saveParticipantToSupabaseDirect(newLocalParticipant);

      const [serverResult, supabaseResult] = await Promise.allSettled([serverPromise, supabasePromise]);

      let serverSucceeded = false;
      let supabaseSucceeded = false;

      if (serverResult.status === 'fulfilled' && serverResult.value && serverResult.value.ok) {
        try {
          const data = await serverResult.value.json();
          participantData = data.participant || newLocalParticipant;
      ...
      if (supabaseResult.status === 'fulfilled' && supabaseResult.value === true) {
      ...
      // If either server or Supabase succeeded, registration was successful!
      if (!serverSucceeded && !supabaseSucceeded) {
        let errMsg = 'Não foi possível concluir o cadastro. Verifique sua conexão e tente novamente.';
        if (serverResult.status === 'fulfilled' && serverResult.value && !serverResult.value.ok) {
          try {
            const errJson = await serverResult.value.json();
      ...
      // Guarantee background direct persistence to Supabase if server alone was first
