...
export const Dashboard: React.FC = () => {
  ...
  const fetchStats = async (isSilent = false) => {
    ...
    try {
      ...
      if (!data) {
        // Fallback: calculate stats directly from /api/participants or localStorage
          ...
          if (stored) {
            try {
              ...
              if (Array.isArray(parsed)) {
                ...
                participants = parsed.filter((p: any) => p && p.id && !legacyIds.has(String(p.id)));
              }
            } catch {}
          }
        }

        let settings = { ticketPrice: 25, revenueGoal: 2500 };
        const settingsStored = localStorage.getItem('ccb_gincana_settings');
        if (settingsStored) {
          try {
        ...
        const ticketPrice = settings.ticketPrice || 25;
