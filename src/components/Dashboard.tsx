...
export const Dashboard: React.FC = () => {
  ...
  const fetchStats = async (isSilent = false) => {
    const now = Date.now();
    if (isSilent && now - lastDashboardFetchRef.current < 45000) {
      return;
    }
    ...
    try {
      ...
      try {
        ...
        if (res.ok) {
          data = await res.json();
          try {
            localStorage.setItem('ccb_gincana_stats_cache', JSON.stringify({ data, timestamp: Date.now() }));
          } catch {}
        }
      } catch (e) {}
      ...
      if (!data) {
        try {
          const raw = localStorage.getItem('ccb_gincana_stats_cache');
          if (raw) {
  ...
  useEffect(() => {
    ...
    // Periodic refresh every 90 seconds when tab is active (preserving free tier egress)
