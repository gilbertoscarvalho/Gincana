...
export const ParticipantManagement: React.FC<ParticipantManagementProps> = ({
  ...
  const handleUpdateStatus = async (id: string, newStatus: ProofStatus) => {
    try {
      ...
      if (res.ok) {
        ...
        try {
          window.dispatchEvent(new CustomEvent('ccb_participant_registered', { detail: { id, proofStatus: newStatus } }));
        } catch (e) {}
  ...
  const handleSaveEdit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        let res = await fetch(`/api/participants/${editingParticipant.id}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
        ...
        if (!res.ok) {
          // Fallback to /participants/:id or POST
          try {
            const fallbackRes = await fetch(`/participants/${editingParticipant.id}`, {
              method: 'PUT',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify(payload)
            });
        ...
        if (res.ok) {
          ...
          if (data && data.participant) {
            updatedItem = data.participant;
          }
        }
      } catch (netErr) {
        console.warn('Notice: Server PUT offline, applying changes locally:', netErr);
      }
      ...
      try {
        window.dispatchEvent(new CustomEvent('ccb_participant_registered', { detail: updatedItem }));
      } catch (e) {}
  ...
  const handleDelete = async (id: string, name: string) => {
    ...
    // 1. Immediately delete from UI and localStorage
    setParticipants((prev) => {
    ...
    try {
      window.dispatchEvent(new CustomEvent('ccb_participant_registered', { detail: { id, deleted: true } }));
    } catch (e) {}
    ...
    try {
      ...
      if (!res.ok) {
        try {
          await fetch(`/participants/${id}`, { method: 'DELETE' });
        } catch {}
      }
    } catch (err) {
      console.warn('Notice: Server delete background sync:', err);
    }
  };
  ...
  // Export to Formatted Excel Table (.xls)
  const exportToExcelTable = () => {
    if (filteredParticipants.length === 0) return;

    const emissionDate = new Date().toLocaleString('pt-BR');
    const totalCount = filteredParticipants.length;
    const gincanaCount = filteredParticipants.filter(p => p.activities?.gincana).length;
    const tocataCount = filteredParticipants.filter(p => p.activities?.tocata).length;
    const withProofCount = filteredParticipants.filter(p => Boolean(p.proofUrl)).length;
    const approvedCount = filteredParticipants.filter(p => p.proofStatus === 'Aprovado').length;

    const rowsHtml = filteredParticipants.map((p, idx) => {
      ...
      const statusStyle =
        p.proofStatus === 'Aprovado'
          ...
          : 'background-color: #ffedd5; color: #c2410c; font-weight: bold;';

      return `
        <tr style="background-color: ${isEven ? '#ffffff' : '#f8fafc'};">
          <td style="border: 1px solid #cbd5e1; padding: 8px; text-align: center; font-weight: bold; color: #64748b;">${idx + 1}</td>
          <td style="border: 1px solid #cbd5e1; padding: 8px; font-weight: bold; color: #0f172a;">${p.fullName}</td>
          <td style="border: 1px solid #cbd5e1; padding: 8px; color: #334155;">${p.congregation}</td>
          <td style="border: 1px solid #cbd5e1; padding: 8px; text-align: center; color: #334155;">${p.age} anos</td>
          <td style="border: 1px solid #cbd5e1; padding: 8px; text-align: center; font-weight: ${p.activities?.gincana ? 'bold' : 'normal'}; color: ${p.activities?.gincana ? '#0284c7' : '#94a3b8'};">${p.activities?.gincana ? 'Sim' : 'Não'}</td>
          <td style="border: 1px solid #cbd5e1; padding: 8px; text-align: center; font-weight: ${p.activities?.tocata ? 'bold' : 'normal'}; color: ${p.activities?.tocata ? '#7c3aed' : '#94a3b8'};">${p.activities?.tocata ? 'Sim' : 'Não'}</td>
          <td style="border: 1px solid #cbd5e1; padding: 8px; color: #334155;">${p.activities?.instrument || '-'}</td>
          <td style="border: 1px solid #cbd5e1; padding: 8px; color: #334155;">${p.foodOrDrink || '-'}</td>
          <td style="border: 1px solid #cbd5e1; padding: 8px; text-align: center; font-weight: bold; color: ${p.proofUrl ? '#15803d' : '#ea580c'};">${p.proofUrl ? 'Sim' : 'Não'}</td>
          <td style="border: 1px solid #cbd5e1; padding: 8px; text-align: center; ${statusStyle}">${p.proofStatus}</td>
          <td style="border: 1px solid #cbd5e1; padding: 8px; color: #64748b; font-size: 10pt;">${p.notes || '-'}</td>
        </tr>
      `;
    }).join('');
    ...
    const excelHtml = `
      ...
      <head>
        ...
        <style>
          body { font-family: Calibri, 'Segoe UI', Arial, sans-serif; font-size: 11pt; color: #0f172a; margin: 20px; }
          .header-box { background-color: #1e293b; color: #ffffff; padding: 18px; text-align: center; border-radius: 8px; margin-bottom: 12px; }
          .title { font-size: 16pt; font-weight: bold; color: #fef08a; margin: 0; }
          .subtitle { font-size: 10pt; color: #cbd5e1; margin-top: 4px; }
          .kpi-row { margin-bottom: 16px; width: 100%; border-collapse: collapse; }
          .kpi-cell { border: 1px solid #cbd5e1; background-color: #f1f5f9; padding: 10px; text-align: center; }
          .kpi-num { font-size: 14pt; font-weight: bold; color: #0f172a; }
          .kpi-label { font-size: 9pt; color: #64748b; text-transform: uppercase; }
          table.data-table { width: 100%; border-collapse: collapse; margin-top: 10px; }
          table.data-table th { background-color: #0f172a; color: #ffffff; border: 1px solid #334155; padding: 10px 8px; font-size: 10pt; text-align: left; }
          table.data-table th.center { text-align: center; }
          .footer-summary { background-color: #e2e8f0; font-weight: bold; }
        </style>
      </head>
      <body>
        <div class="header-box">
          <h1 class="title">SOMOS JÓIAS PRECIOSAS 2026 • RELATÓRIO GERAL DE INSCRITOS</h1>
          <p class="subtitle">Gincana & Tocata • Emissão: ${emissionDate} • Total listado: ${totalCount} participante(s)</p>
        </div>

        <table class="kpi-row">
          <tr>
            <td class="kpi-cell"><div class="kpi-num">${totalCount}</div><div class="kpi-label">Total Inscritos</div></td>
            <td class="kpi-cell"><div class="kpi-num" style="color: #0284c7;">${gincanaCount}</div><div class="kpi-label">Na Gincana</div></td>
            <td class="kpi-cell"><div class="kpi-num" style="color: #7c3aed;">${tocataCount}</div><div class="kpi-label">Na Tocata</div></td>
            <td class="kpi-cell"><div class="kpi-num" style="color: #15803d;">${withProofCount}</div><div class="kpi-label">Com Comprovante</div></td>
            <td class="kpi-cell"><div class="kpi-num" style="color: #16a34a;">${approvedCount}</div><div class="kpi-label">Aprovados</div></td>
          </tr>
        </table>
        ...
        <table class="data-table">
          <thead>
            <tr>
              <th class="center" style="width: 40px;">Nº</th>
              <th>Nome Completo</th>
              <th>Comum Congregação</th>
              <th class="center" style="width: 70px;">Idade</th>
              <th class="center" style="width: 75px;">Gincana</th>
              <th class="center" style="width: 75px;">Tocata</th>
              <th>Instrumento Musical</th>
              <th>Alimento / Bebida</th>
              <th class="center" style="width: 95px;">Comprovante</th>
          ...
          <tfoot>
            <tr class="footer-summary">
              <td colspan="3" style="border: 1px solid #cbd5e1; padding: 10px; font-weight: bold;">TOTAL: ${totalCount} PARTICIPANTE(S)</td>
              <td style="border: 1px solid #cbd5e1; padding: 10px; text-align: center;">-</td>
              <td style="border: 1px solid #cbd5e1; padding: 10px; text-align: center; color: #0284c7;">${gincanaCount}</td>
              <td style="border: 1px solid #cbd5e1; padding: 10px; text-align: center; color: #7c3aed;">${tocataCount}</td>
              <td colspan="2" style="border: 1px solid #cbd5e1; padding: 10px;">-</td>
              <td style="border: 1px solid #cbd5e1; padding: 10px; text-align: center; color: #15803d;">${withProofCount}</td>
              <td style="border: 1px solid #cbd5e1; padding: 10px; text-align: center; color: #16a34a;">${approvedCount} Aprov.</td>
    ...
    link.href = url;
    link.setAttribute('download', `tabela_inscritos_joias_preciosas_${new Date().toISOString().slice(0, 10)}.xls`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  };

  // Print Table Report (A4 optimized layout for printing & PDF export)
  const printTableReport = () => {
    ...
    const rowsHtml = filteredParticipants.map((p, idx) => {
      ...
      return `
        <tr style="background-color: ${isEven ? '#ffffff' : '#f9fafb'};">
          ...
          <td style="font-weight: 600; color: #111827;">${p.fullName}</td>
          <td>${p.congregation}</td>
          <td style="text-align: center;">${p.age}</td>
          <td style="text-align: center; font-weight: bold; color: ${p.activities?.gincana ? '#0284c7' : '#9ca3af'};">${p.activities?.gincana ? 'Sim' : 'Não'}</td>
          <td style="text-align: center; font-weight: bold; color: ${p.activities?.tocata ? '#7c3aed' : '#9ca3af'};">${p.activities?.tocata ? 'Sim' : 'Não'}</td>
          <td>${p.activities?.instrument || '-'}</td>
          <td>${p.foodOrDrink || '-'}</td>
          <td style="text-align: center; font-weight: bold; color: ${p.proofUrl ? '#15803d' : '#ea580c'};">${p.proofUrl ? 'Sim' : 'Não'}</td>
          <td style="text-align: center; font-weight: bold;">${p.proofStatus}</td>
          <td style="font-size: 8.5pt; color: #6b7280;">${p.notes || '-'}</td>
        </tr>
      `;
    }).join('');
    ...
    const printHtml = `
      ...
      <head>
        <meta charset="utf-8">
        <title>Relatório de Inscritos • Somos Jóias Preciosas 2026</title>
        <style>
          @page { size: A4 landscape; margin: 12mm 10mm 12mm 10mm; }
          body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; font-size: 9.5pt; color: #111827; margin: 0; padding: 15px; }
          ...
          .title { font-size: 15pt; font-weight: 800; color: #0f172a; margin: 0; text-transform: uppercase; }
          .subtitle { font-size: 9pt; color: #4b5563; margin-top: 3px; }
          .kpis { display: flex; gap: 10px; margin-bottom: 12px; }
          .kpi-card { flex: 1; border: 1px solid #e5e7eb; border-radius: 6px; padding: 8px; text-align: center; background: #f9fafb; }
          .kpi-val { font-size: 13pt; font-weight: 800; color: #0f172a; }
          .kpi-lbl { font-size: 7.5pt; color: #6b7280; text-transform: uppercase; font-weight: 600; }
          table { width: 100%; border-collapse: collapse; margin-top: 5px; }
          th { background-color: #1e293b; color: #ffffff; border: 1px solid #334155; padding: 6px 5px; font-size: 8.5pt; text-align: left; }
          td { border: 1px solid #d1d5db; padding: 5px; font-size: 8.5pt; vertical-align: middle; }
      ...
      <body>
        <div class="no-print">
          <button onclick="window.print()" style="padding: 8px 16px; background: #0f172a; color: #fff; font-weight: bold; border-radius: 6px; cursor: pointer; border: none;">
            🖨️ Imprimir / Salvar como PDF
          </button>
        </div>
        <div class="header">
          <div>
            <h1 class="title">Somos Jóias Preciosas 2026 • Relatório de Inscritos</h1>
            <div class="subtitle">Gincana & Tocata CCB • Emissão: ${emissionDate}</div>
          </div>
          <div style="font-size: 9pt; font-weight: bold; color: #0f172a;">
            Total: ${totalCount} Inscritos
          </div>
        </div>

        <div class="kpis">
          <div class="kpi-card"><div class="kpi-val">${totalCount}</div><div class="kpi-lbl">Total de Inscritos</div></div>
          <div class="kpi-card"><div class="kpi-val" style="color: #0284c7;">${gincanaCount}</div><div class="kpi-lbl">Na Gincana</div></div>
          <div class="kpi-card"><div class="kpi-val" style="color: #7c3aed;">${tocataCount}</div><div class="kpi-lbl">Na Tocata</div></div>
          <div class="kpi-card"><div class="kpi-val" style="color: #15803d;">${withProofCount}</div><div class="kpi-lbl">Com Comprovante</div></div>
          <div class="kpi-card"><div class="kpi-val" style="color: #16a34a;">${approvedCount}</div><div class="kpi-lbl">Aprovados</div></div>
        </div>
        ...
        <table>
          <thead>
            <tr>
              ...
              <th style="width: 50px; text-align: center;">Gincana</th>
              <th style="width: 50px; text-align: center;">Tocata</th>
              <th>Instrumento</th>
              <th>Alimento / Bebida</th>
              <th style="width: 65px; text-align: center;">Comprov.</th>
  ...
  return (
    <div className="max-w-7xl mx-auto my-6 px-4 space-y-6">
      ...
      {globalBanner && (
        <div
          className={`p-4 rounded-2xl flex items-center justify-between shadow-lg transition-all animate-fade-in ${
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button
              ...
              className="px-3.5 py-2.5 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-200 font-bold text-xs rounded-xl transition-all shadow-md flex items-center gap-2 disabled:opacity-50 cursor-pointer"
              title="Imprimir ou Salvar em PDF em formato de tabela"
            >
            ...
            <button
              ...
              title="Exportar tabela formatada para Microsoft Excel (.xls)"
            >
            ...
            <button
              onClick={exportToCSV}
              disabled={filteredParticipants.length === 0}
              className="px-3 py-2.5 bg-slate-800 hover:bg-slate-700 text-slate-400 hover:text-slate-200 font-medium text-xs rounded-xl transition-all border border-slate-700/60 flex items-center gap-1.5 disabled:opacity-50 cursor-pointer"
