...
export const ParticipantManagement: React.FC<ParticipantManagementProps> = ({
  ...
  // Add Participant Modal State (Admin direct registration)
  const [showAddModal, setShowAddModal] = useState(false);
  const [addFullName, setAddFullName] = useState('');
  const [addCongregation, setAddCongregation] = useState('');
  const [addCustomCongregation, setAddCustomCongregation] = useState('');
  const [addAge, setAddAge] = useState('18');
  const [addFoodOrDrink, setAddFoodOrDrink] = useState('');
  const [addParticipatesGincana, setAddParticipatesGincana] = useState(true);
  const [addParticipatesTocata, setAddParticipatesTocata] = useState(false);
  const [addInstrument, setAddInstrument] = useState('');
  const [addAmountPaid, setAddAmountPaid] = useState('20.00');
  const [addProofStatus, setAddProofStatus] = useState<ProofStatus>('Pendente');
  ...
  const fetchParticipants = async (isSilent: any = false, forceFresh = false) => {
    const silent = typeof isSilent === 'boolean' ? isSilent : false;
    const force = Boolean(forceFresh || !silent);

    const now = Date.now();
    if (!force && silent && now - lastParticipantsFetchRef.current < 30000) {
      return;
    }
    ...
    try {
      const directPromise = fetchParticipantsFromSupabaseDirect(force);
      const apiPromise = fetch('/api/participants')
        .then((r) => (r.ok ? r.json() : null))
        .catch(() => null);

      const [directList, apiList] = await Promise.all([directPromise, apiPromise]);

      let finalSource: Participant[] = [];
      if (Array.isArray(directList) && directList.length > 0) {
        finalSource = directList;
      } else if (Array.isArray(apiList) && apiList.length > 0) {
        finalSource = apiList;
      } else if (Array.isArray(directList)) {
        finalSource = directList;
      } else if (Array.isArray(apiList)) {
        finalSource = apiList;
      }

      const sortedList = [...finalSource].sort((a, b) => {
      ...
      console.warn('Notice: Participants sync error:', err);
    } finally {
  ...
  useEffect(() => {
    fetchParticipants(false, true);

    // Periodic background sync every 60 seconds
    const intervalId = setInterval(() => {
      if (document.visibilityState === 'visible') {
        fetchParticipants(true, false);
      }
    }, 60000);

    const handleFocusOrVisibility = () => {
      if (document.visibilityState === 'visible') {
        fetchParticipants(true, true);
      }
    };

    window.addEventListener('focus', handleFocusOrVisibility);
    document.addEventListener('visibilitychange', handleFocusOrVisibility);
    window.addEventListener('ccb_participant_registered', () => fetchParticipants(true, true));
    window.addEventListener('storage', () => fetchParticipants(true, true));
    ...
    try {
      ...
      if (supabase) {
        channel = supabase
          .channel('participants_realtime_channel')
    ...
    return () => {
      ...
      window.removeEventListener('ccb_participant_registered', () => fetchParticipants(true, true));
      window.removeEventListener('storage', () => fetchParticipants(true, true));
      if (channel) {
        try {
          const supabase = getSupabaseClient();
  ...
  const handleSaveEdit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const adminPass = sessionStorage.getItem('ccb_admin_session_pass') || 'ccb*2026';
      ...
      // 1. Direct Supabase write (primary cloud truth)
      const directSuccess = await saveParticipantToSupabaseDirect(updatedItem).catch(() => false);

      // 2. Server API write with Admin headers
      let res = await fetch(`/api/participants/${encodeURIComponent(editingParticipant.id)}`, {
        ...
        body: JSON.stringify({ ...payload, adminPassword: adminPass })
      });
      ...
      if (!res.ok && !directSuccess) {
        throw new Error('Não foi possível salvar a edição no servidor.');
      }
      ...
      setGlobalBanner({
        type: 'success',
        message: `Cadastro de "${cleanFullName}" atualizado com sucesso no Supabase!`
      });
  ...
  const handleDelete = async (id: string, name: string) => {
    ...
    try {
      ...
      // 1. Direct deletion from Supabase table
      const directDeleted = await deleteParticipantFromSupabaseDirect(id).catch(() => false);

      // 2. Server API deletion with Admin headers
      let res = await fetch(`/api/participants/${encodeURIComponent(id)}`, { 
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          'x-admin-password': adminPass
      ...
      if (!res.ok && !directDeleted) {
        try {
          const fallbackRes = await fetch(`/api/participants/${encodeURIComponent(id)}/delete`, { 
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'x-admin-password': adminPass
  ...
  const handleOpenAddModal = () => {
    setAddFullName('');
    setAddCongregation(activeCongregationsList[0] || 'Central');
    setAddCustomCongregation('');
    setAddAge('18');
    setAddFoodOrDrink('');
    setAddParticipatesGincana(true);
    setAddParticipatesTocata(false);
    setAddInstrument('');
    setAddAmountPaid('20.00');
    setAddProofStatus('Pendente');
    setAddNotes('');
    setAddModalError(null);
    setShowAddModal(true);
  };

  // Admin: Save New Participant
  const handleSaveNewParticipant = async (e?: React.FormEvent) => {
    if (e) e.preventDefault();
    const cleanFullName = addFullName.trim();
    if (!cleanFullName) {
      setAddModalError('Por favor, informe o nome completo do participante.');
      return;
    }
    const cleanAge = parseInt(addAge, 10);
    if (isNaN(cleanAge) || cleanAge < 1 || cleanAge > 120) {
      setAddModalError('Por favor, informe uma idade válida (entre 1 e 120 anos).');
      return;
    }

    const finalCongregation = addCongregation === 'OUTRA'
      ? (addCustomCongregation.trim() || 'Outra')
      : (addCongregation || activeCongregationsList[0] || 'Central');

    const parsedAmount = parseFloat(addAmountPaid.replace(',', '.'));
    const finalAmountPaid = !isNaN(parsedAmount) ? parsedAmount : 20.0;
    ...
    try {
      ...
      const newParticipant: Participant = {
        ...
        congregation: finalCongregation,
        age: cleanAge,
        foodOrDrink: addFoodOrDrink.trim(),
        activities: {
          gincana: addParticipatesGincana,
          tocata: addParticipatesTocata,
          instrument: addParticipatesTocata ? addInstrument.trim() : ''
        },
        proofUrl: null,
        proofFileName: null,
        proofFileType: null,
        proofStatus: addProofStatus,
      ...
      const directSuccess = await saveParticipantToSupabaseDirect(newParticipant).catch(() => false);
      ...
      let res = await fetch('/api/participants', {
        ...
        body: JSON.stringify({ ...newParticipant, adminPassword: adminPass })
      });

      if (!res.ok && !directSuccess) {
        throw new Error('Falha ao registrar novo participante no servidor.');
      }

      setParticipants((prev) => [newParticipant, ...prev]);

      try {
        window.dispatchEvent(new CustomEvent('ccb_participant_registered', { detail: newParticipant }));
      } catch {}

      setGlobalBanner({
        type: 'success',
        message: `Participante "${cleanFullName}" cadastrado com sucesso!`
      });
      setTimeout(() => setGlobalBanner(null), 5000);
      ...
      console.error('Error saving new participant:', err);
      setAddModalError(err?.message || 'Erro ao cadastrar participante.');
    } finally {
  ...
  return (
    <div className="max-w-7xl mx-auto my-6 px-4 space-y-6 print:p-0 print:m-0 print:max-w-none">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl space-y-4 print:hidden">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            {/* Novo Participante (Admin Direct) */}
            <button
              onClick={handleOpenAddModal}
              className="px-4 py-2.5 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-slate-950 font-extrabold text-xs rounded-xl transition-all shadow-lg flex items-center gap-2 cursor-pointer shadow-amber-500/20"
              title="Cadastrar novo participante diretamente pelo painel administrativo"
            >
      ...
      {/* Add Participant Modal (Admin Direct) */}
      {showAddModal && (
        <div className="fixed inset-0 z-50 bg-slate-950/85 backdrop-blur-md flex items-center justify-center p-2 sm:p-4 overflow-y-auto">
          <div className="bg-slate-900 border border-amber-500/40 rounded-3xl max-w-2xl w-full my-auto shadow-2xl overflow-hidden flex flex-col max-h-[92vh]">
            {/* Modal Header */}
            <div className="p-4 sm:p-5 border-b border-slate-800 flex items-center justify-between bg-slate-950/60">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400">
                    Inclusão direta no banco de dados Supabase com senha de Administrador
                  </p>
                </div>
              </div>
              <button
            ...
            {/* Modal Body / Form */}
            <form onSubmit={handleSaveNewParticipant} className="overflow-y-auto flex-1 p-4 sm:p-6 space-y-4">
              {addModalError && (
                <div className="p-3 bg-rose-500/15 border border-rose-500/40 rounded-xl flex items-center gap-2.5 text-xs text-rose-200">
                ...
                <div className="sm:col-span-2 space-y-1">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full bg-slate-950 border border-slate-800 focus:border-amber-500 rounded-xl pl-9 pr-3.5 py-2 text-xs text-slate-100 placeholder-slate-500 focus:outline-none"
                    />
                  </div>
                </div>
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  <Church className="absolute left-3 top-2.5 w-4 h-4 text-slate-500" />
                  <select
                    value={addCongregation}
                    onChange={(e) => setAddCongregation(e.target.value)}
                    className="w-full bg-slate-950 border border-slate-800 focus:border-amber-500 rounded-xl pl-9 pr-8 py-2 text-xs text-slate-100 focus:outline-none appearance-none cursor-pointer"
                  >
                    {activeCongregationsList.map((cong) => (
                      <option key={cong} value={cong} className="bg-slate-900 text-slate-100">
                        {cong}
                      </option>
                    ))}
                    <option value="OUTRA" className="bg-slate-900 text-amber-300 font-semibold">
                ...
                {addCongregation === 'OUTRA' && (
                  <input
                    type="text"
                    required
                    value={addCustomCongregation}
                    onChange={(e) => setAddCustomCongregation(e.target.value)}
                    placeholder="Digite o nome da sua congregação"
                    className="w-full mt-2 bg-slate-950 border border-amber-500/50 rounded-xl px-3.5 py-2 text-xs text-slate-100 placeholder-slate-500 focus:outline-none"
                  />
                )}
              </div>

              {/* 3. Atividades (Gincana & Tocata) */}
              <div className="space-y-3 pt-2 border-t border-slate-800">
                <h4 className="text-xs font-bold uppercase tracking-wider text-amber-300 flex items-center gap-1.5">
                  <Sparkles className="w-3.5 h-3.5" /> Atividades do Evento
                </h4>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                  <label className="flex items-center gap-3 p-3 rounded-2xl bg-slate-950 border border-slate-800 hover:border-slate-700 cursor-pointer transition-colors">
                    <input
                      type="checkbox"
                      checked={addParticipatesGincana}
                      onChange={(e) => setAddParticipatesGincana(e.target.checked)}
                      className="w-4 h-4 rounded text-amber-500 focus:ring-amber-500 focus:ring-offset-slate-900 bg-slate-900 border-slate-700"
                    />
                    <div>
                      <div className="text-xs font-bold text-white flex items-center gap-1">
                        <Flag className="w-3.5 h-3.5 text-amber-400" /> Participar da Gincana
                      </div>
                      <div className="text-[11px] text-slate-400">Brincadeiras e dinâmicas de equipe</div>
                    </div>
                  </label>
                  ...
                  <label className="flex items-center gap-3 p-3 rounded-2xl bg-slate-950 border border-slate-800 hover:border-slate-700 cursor-pointer transition-colors">
                    <input
                      type="checkbox"
                      checked={addParticipatesTocata}
                      onChange={(e) => setAddParticipatesTocata(e.target.checked)}
                      className="w-4 h-4 rounded text-sky-500 focus:ring-sky-500 focus:ring-offset-slate-900 bg-slate-900 border-slate-700"
                    />
                    <div>
                      <div className="text-xs font-bold text-white flex items-center gap-1">
                        <Music className="w-3.5 h-3.5 text-sky-400" /> Participar da Tocata
              ...
              {/* 4. Alimento ou Bebida */}
              <div className="space-y-2 pt-2 border-t border-slate-800">
                <label className="block text-xs font-semibold text-slate-300">
                  Alimento / Bebida para o Evento (opcional)
                </label>
                <div className="relative">
                  <Utensils className="absolute left-3 top-2.5 w-4 h-4 text-purple-400" />
                  <input
                    type="text"
                    value={addFoodOrDrink}
                    onChange={(e) => setAddFoodOrDrink(e.target.value)}
                    placeholder="Ex: Suco de Maracujá 2L, Bolo, Refrigerante..."
                    className="w-full bg-slate-950 border border-slate-800 focus:border-purple-500 rounded-xl pl-9 pr-3.5 py-2 text-xs text-slate-100 placeholder-slate-500 focus:outline-none"
                  />
                </div>
              </div>

              {/* 5. Valor Pago e Status Comprovante */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2 border-t border-slate-800">
                <div className="space-y-1">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder="20,00"
                      className="w-full bg-slate-950 border border-slate-800 focus:border-emerald-500 rounded-xl pl-9 pr-3.5 py-2 text-xs text-slate-100 focus:outline-none"
                    />
                  </div>
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <select
                    value={addProofStatus}
                    onChange={(e) => setAddProofStatus(e.target.value as ProofStatus)}
                    className="w-full bg-slate-950 border border-slate-800 focus:border-amber-500 rounded-xl px-3 py-2 text-xs text-slate-100 focus:outline-none cursor-pointer"
                  >
              ...
              <div className="space-y-1 pt-2 border-t border-slate-800">
                ...
                <textarea
                  ...
                  placeholder="Anotações internas sobre pagamento, restrições alimentares, etc."
                  className="w-full bg-slate-950 border border-slate-800 focus:border-amber-500 rounded-xl p-3 text-xs text-slate-100 placeholder-slate-500 focus:outline-none resize-none"
                />
              </div>
              ...
              <div className="pt-3 border-t border-slate-800 flex items-center justify-end gap-2.5">
                <button
                  ...
                  className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-semibold rounded-xl transition-colors cursor-pointer disabled:opacity-50"
                >
                  Cancelar
                </button>
                <button
                  type="submit"
                  disabled={isSavingNew}
                  className="px-5 py-2 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-slate-950 text-xs font-bold rounded-xl transition-all shadow-md flex items-center gap-2 cursor-pointer disabled:opacity-50"
                >
                  {isSavingNew ? (
                    <>
                      <span className="w-3.5 h-3.5 border-2 border-slate-950 border-t-transparent rounded-full animate-spin" />
                      Cadastrando no Supabase...
                    </>
                  ) : (
                    <>
