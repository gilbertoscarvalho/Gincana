...
export const ParticipantManagement: React.FC<ParticipantManagementProps> = ({
  ...
  const fetchParticipants = async (isSilent = false) => {
    ...
    try {
      // 1. Fetch from server with explicit cache-busting headers
      ...
      if (serverSuccess) {
        ...
        const sortedList = [...serverParticipants].sort((a, b) => {
        ...
        try {
          localStorage.setItem('ccb_gincana_local_participants', JSON.stringify(sortedList));
        } catch (e) {}
      } else if (!isSilent) {
        // Fallback to localStorage only when offline / initial server network error
        ...
        try {
          ...
          if (stored) {
            ...
            if (Array.isArray(parsed)) {
              const legacyIds = new Set(['p-101', 'p-102', 'p-103', 'p-104', 'p-105', 'p-test-1']);
              localParticipants = parsed.filter((p: any) => p && p.id && !legacyIds.has(String(p.id)));
            }
          }
        } catch (e) {}

        const sortedLocal = [...localParticipants].sort((a, b) => {
          const dateA = new Date(a.createdAt || 0).getTime();
          const dateB = new Date(b.createdAt || 0).getTime();
          return dateB - dateA;
        });
        setParticipants(sortedLocal);
      }
    } catch (err) {
      console.warn('Notice: Background participants sync check:', err);
    } finally {
  ...
  useEffect(() => {
    ...
    // Continuous polling every 4 seconds so any registrations from mobile or desktop appear immediately
  ...
  const handleUpdateStatus = async (id: string, newStatus: ProofStatus) => {
    ...
    setGlobalBanner({
      type: 'success',
      message: `Status do comprovante atualizado para "${newStatus}".`
    });
    setTimeout(() => setGlobalBanner(null), 4000);
    onDataChanged();

    // 2. Server persistence with fallbacks
    try {
      const payload = {
        id,
