...
export const Home: React.FC<HomeProps> = ({
  onGoToRegister,
  onGoToLookup,
  eventDateISO = '2026-09-07T11:00:00.000Z',
  ...
  useEffect(() => {
    ...
    if (isNaN(targetDateMs)) {
      ...
      targetDateMs = new Date('2026-09-07T08:00:00-03:00').getTime();
    }
    ...
    <div className="space-y-12 pb-12">
      ...
      <section className="relative overflow-hidden bg-gradient-to-b from-slate-900 via-slate-900 to-slate-950 border-b border-amber-500/20 py-12 sm:py-20 px-4">
        ...
        <div className="max-w-5xl mx-auto text-center relative z-10 space-y-6">
          ...
          {/* Interactive Music / Hymn Player Widget (Tocar Música / Google Drive / Arquivo) */}
          {onTogglePlayAudio && backgroundMusicUrl && backgroundMusicUrl.trim() !== '' && (
            <div className="pt-4 max-w-2xl mx-auto">
              <div className="bg-slate-900/95 border border-amber-500/30 rounded-3xl p-4 sm:p-5 shadow-2xl backdrop-blur-md flex flex-col sm:flex-row items-center justify-between gap-4">
                <div className="flex items-center gap-3.5 w-full sm:w-auto overflow-hidden">
                  <button
                    type="button"
                    onClick={onTogglePlayAudio}
                    title={isPlayingAudio ? 'Pausar Hino' : 'Tocar Hino do Evento'}
                    className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 shadow-lg transition-all cursor-pointer ${
                      isPlayingAudio
                        ? 'bg-amber-400 text-slate-950 shadow-amber-500/30 scale-105 animate-pulse'
                  ...
                  <div className="overflow-hidden text-left flex-1">
                    <div className="flex items-center gap-2">
                      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-amber-500/20 text-amber-300 border border-amber-500/30">
                        ...
                        {isPlayingAudio ? 'Reproduzindo Agora' : 'Hino do Evento'}
                      </span>
                      {backgroundMusicUrl && (
                        <span className="text-[10px] text-slate-400 font-mono hidden sm:inline">
                          {backgroundMusicUrl.includes('drive.google.com') || backgroundMusicUrl.includes('drive.usercontent.google.com')
                            ? 'Google Drive'
                            : backgroundMusicUrl.startsWith('data:') || backgroundMusicUrl.startsWith('/uploads/')
                            ? 'Arquivo Gravado'
                            : 'Áudio Web'}
                        </span>
                      )}
                    ...
                    <h3 className="text-sm font-bold text-white truncate mt-0.5">
                      {backgroundMusicTitle || 'Hino do Evento'}
                    </h3>
                    <p className="text-[11px] text-slate-400 truncate">
                      {isPlayingAudio ? 'Ouça durante a navegação pelo portal' : 'Clique no play para ouvir o hino'}
                    </p>
                  </div>
                </div>

                {/* Right controls: volume & quick mute */}
                <div className="flex items-center gap-3 w-full sm:w-auto justify-end border-t sm:border-0 border-slate-800/80 pt-2 sm:pt-0">
                  {onToggleMuteAudio && (
                    <button
                      type="button"
                      onClick={onToggleMuteAudio}
                      className="p-2 rounded-xl bg-slate-950 border border-slate-800 text-slate-300 hover:text-amber-300 transition-colors"
                      title={isMutedAudio ? 'Desmutar' : 'Mutar'}
                    >
                      {isMutedAudio ? (
          ...
          {filteredMedia.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredMedia.map((item) => {
                const isFolder = item.url.includes('/folders/') || item.url.includes('drive.google.com/drive/u/0/folders');
                return (
                  <div
                    key={item.id}
                  ...
                  >
                    <div className="relative h-48 overflow-hidden bg-slate-900 flex items-center justify-center">
                      {isFolder ? (
                        <div className="w-full h-full bg-gradient-to-br from-slate-900 to-amber-950/40 flex flex-col items-center justify-center p-4 text-center group-hover:scale-105 transition-transform duration-500">
                          <div className="w-14 h-14 rounded-2xl bg-amber-500/20 text-amber-400 flex items-center justify-center mb-2 border border-amber-500/30">
                            <FolderOpen className="w-7 h-7" />
                          </div>
                          <span className="text-xs font-bold text-amber-300">Álbum no Google Drive</span>
                          <span className="text-[10px] text-slate-400 mt-0.5">Fotos & Gravações do Evento</span>
                        </div>
                      ...
                      <span className="absolute top-3 left-3 px-2.5 py-0.5 rounded-md bg-slate-900/80 backdrop-blur-md text-amber-300 text-[10px] font-bold uppercase tracking-wider border border-slate-700">
                        {isFolder ? 'Álbum Drive' : item.type === 'video' ? 'Vídeo' : 'Foto'}
                      </span>
                    </div>
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/90 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-amber-500/30 rounded-3xl max-w-3xl w-full p-6 space-y-4 relative shadow-2xl">
            <div className="flex justify-between items-center border-b border-slate-800 pb-3">
              <div className="flex items-center gap-2">
                {selectedImageModal.url.includes('/folders/') ? (
            ...
            <div className="rounded-2xl overflow-hidden bg-slate-950 flex justify-center max-h-[450px]">
              {selectedImageModal.url.includes('/folders/') || selectedImageModal.url.includes('drive.google.com/drive/u/0/folders') ? (
                <div className="p-8 text-center space-y-4 w-full">
                  ...
                  <div>
                    <h4 className="text-base font-bold text-white">Pasta de Fotos & Vídeos no Google Drive</h4>
                    <p className="text-xs text-slate-400 mt-1 max-w-md mx-auto">
                      Esta pasta contém a galeria completa de fotos e vídeos hospedados na nuvem do Google Drive.
                    </p>
                  </div>
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center gap-2 px-6 py-3 bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold text-xs rounded-xl shadow-lg shadow-amber-500/20 transition-all cursor-pointer"
                  >
                    <ExternalLink className="w-4 h-4" />
                    Abrir Pasta Completa no Google Drive
                  </a>
                </div>
            ...
            <div className="flex items-center justify-between pt-2">
              ...
              {selectedImageModal.url && !selectedImageModal.url.startsWith('data:') && (
                <a
                  href={selectedImageModal.url}
                  target="_blank"
                  rel="noreferrer"
