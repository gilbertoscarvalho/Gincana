...
export const AdminSettings: React.FC<AdminSettingsProps> = ({
  ...
  const [isTestingCustomSupabase, setIsTestingCustomSupabase] = useState(false);
  ...
  const checkSupabaseHealth = async () => {
    ...
    try {
      ...
      if (health.connected) {
        ...
        // Try server health endpoint if direct query had client-side restriction
        try {
          ...
          if (res.ok) {
            ...
            if (serverStatus.connected) {
              setSupabaseHealth({
                connected: true,
                participantCount: serverStatus.participantsCount || 0,
                projectUrl: serverStatus.url || health.projectUrl
              });
              return;
            }
          }
        ...
        if (res.ok) {
          ...
          if (serverStatus.connected) {
            setSupabaseHealth({
              connected: true,
              participantCount: serverStatus.participantsCount || 0,
              projectUrl: serverStatus.url || 'Supabase'
            });
            return;
          }
        }
      ...
      setSupabaseHealth({
        ...
        participantCount: 0,
        projectUrl: 'Supabase',
        error: e?.message || 'Falha ao consultar status do Supabase'
      });
    } finally {
  ...
  useEffect(() => {
    ...
    if (stored.url) {
      setCustomUrlInput(stored.url);
      setCustomKeyInput(stored.key);
    } else {
      const creds = getSupabaseCredentials();
      if (creds.url && !creds.url.includes('tcmurarerhzrcgfqanhf')) {
        setCustomUrlInput(creds.url);
        setCustomKeyInput(creds.key);
      }
    }
    checkSupabaseHealth();
  }, []);

  const handleConnectCustomSupabase = async () => {
    const cleanUrl = customUrlInput.trim();
    const cleanKey = customKeyInput.trim();

    if (!cleanUrl || !cleanKey) {
      setErrorMsg('Por favor, informe a URL e a Chave de API do Supabase.');
      return;
    }

    setIsTestingCustomSupabase(true);
    setErrorMsg(null);
    try {
      const health = await getSupabaseLiveHealth(cleanUrl, cleanKey);
      if (!health.connected) {
        throw new Error(health.error || 'Falha ao conectar com o Supabase fornecido.');
      }
      ...
      try {
        ...
        console.warn('Server config sync warning:', e);
      }

      setSupabaseHealth(health);
      setSuccessMsg(`Conectado ao Supabase com sucesso! Encontrados ${health.participantCount} participantes no projeto.`);
      setShowSupabaseConfig(false);
    } catch (err: any) {
      setErrorMsg('Erro ao conectar com o Supabase: ' + (err?.message || 'Verifique as credenciais digitadas.'));
    } finally {
      setIsTestingCustomSupabase(false);
    }
  };
  ...
  const handleResetSupabaseCredentials = async () => {
    ...
    setSuccessMsg('Credenciais personalizadas removidas. O sistema voltará a usar as variáveis de ambiente.');
  };
  ...
  const handlePhotoFilesSelected = async (files: FileList) => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        // 1. First try uploading binary directly to Supabase Storage CDN
        const storageUrl = await uploadFileToSupabaseStorage(file, `gal_${Date.now()}_${file.name}`);
        if (storageUrl) {
          finalUrl = storageUrl;
          finalThumb = storageUrl;
        } else {
          // Fallback to server upload proxy
          try {
        ...
        // Fallback only if completely offline: compress image
        if (!finalUrl) {
          if (isVideo) {
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="bg-slate-950 border border-slate-800 p-5 sm:p-6 rounded-2xl space-y-5">
            <div className="border-b border-slate-800 pb-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              ...
              {/* Supabase Status Badge & Diagnostic Info */}
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 self-start sm:self-auto">
                <div className={`px-3 py-1.5 rounded-xl border text-xs font-semibold flex items-center gap-2 ${
                  ...
                  <span className={`w-2 h-2 rounded-full ${supabaseHealth?.connected ? 'bg-emerald-400 animate-pulse' : 'bg-rose-500'}`} />
                  <span>
                    {supabaseHealth?.connected
                      ? `Supabase Conectado (${supabaseHealth.participantCount} inscritos)`
                      : `Supabase Desconectado ${supabaseHealth?.error ? `(${supabaseHealth.error})` : ''}`}
                  </span>
                  {supabaseHealth?.projectUrl && (
                    <span className="text-[10px] px-1.5 py-0.5 rounded bg-slate-900/80 border border-slate-700/60 text-slate-400 font-mono">
                      {supabaseHealth.projectUrl}
                    </span>
                  )}
                </div>
                ...
                <button
                  ...
                  className="px-2.5 py-1.5 bg-slate-900 hover:bg-slate-800 text-amber-300 text-xs font-semibold rounded-lg border border-slate-800 transition-colors flex items-center gap-1.5 cursor-pointer"
                  title="Configurar credenciais do Supabase"
                >
                  <Settings className="w-3.5 h-3.5 text-amber-400" />
                  <span>{showSupabaseConfig ? 'Fechar Configuração' : 'Configurar Conexão'}</span>
                </button>
            ...
            {showSupabaseConfig && (
              <div className="p-4 bg-slate-900/90 border border-amber-500/30 rounded-2xl space-y-3 animate-in fade-in">
                <div className="flex items-center justify-between border-b border-slate-800 pb-2">
                  <div className="flex items-center gap-2">
                    <Database className="w-4 h-4 text-amber-400" />
                    <h4 className="text-xs font-bold text-white">Configurar Projeto do Supabase (Novo Banco de Dados)</h4>
                  </div>
                  <button
                    type="button"
                ...
                <p className="text-[11px] text-slate-300 leading-relaxed">
                  Conecte a aplicação ao seu novo projeto do Supabase inserindo a URL e a Chave de API (anon ou service_role). Os dados informados aqui passam a ser utilizados imediatamente tanto pelo formulário de inscrições quanto pelo backup.
                </p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-slate-950 border border-slate-700 rounded-xl px-3 py-2 text-white text-xs font-mono focus:outline-none focus:border-amber-500"
                    />
                    <span className="text-[10px] text-slate-400 mt-1 block">
                      Encontrado em: Supabase Dashboard &rarr; Project Settings &rarr; Data API &rarr; Project URL
                    </span>
                  </div>

                  <div>
                    <label className="text-[11px] font-bold text-slate-300 block mb-1">
                      API Key (Publishable, Secret ou JWT anon)
                    </label>
                    <input
                      type="password"
                    ...
                    />
                    <span className="text-[10px] text-slate-400 mt-1 block">
                      Aceita chaves públicas (sb_publishable_...) ou token JWT (anon).
                    </span>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap items-center justify-end gap-2 pt-2 border-t border-slate-800">
                  ...
                  <button
                    type="button"
                    onClick={handleConnectCustomSupabase}
                    disabled={isTestingCustomSupabase || !customUrlInput.trim() || !customKeyInput.trim()}
                    className="px-5 py-1.5 rounded-xl bg-amber-500 hover:bg-amber-400 disabled:opacity-50 text-slate-950 text-xs font-bold flex items-center gap-1.5 shadow-md transition-all cursor-pointer"
                  >
                    {isTestingCustomSupabase ? (
                      <RefreshCw className="w-3.5 h-3.5 animate-spin" />
                    ) : (
                      <Check className="w-3.5 h-3.5" />
                    )}
                    <span>{isTestingCustomSupabase ? 'Testando Conexão...' : 'Testar e Conectar ao Supabase'}</span>
                  </button>
                </div>
              </div>
            )}
