...
export const AdminSettings: React.FC<AdminSettingsProps> = ({
  ...
  const handleAudioFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 15 * 1024 * 1024) {
      setErrorMsg('O arquivo de áudio selecionado tem mais de 15MB. Para arquivos muito pesados, recomendamos colar o link do Google Drive no campo de link.');
      return;
    }
    ...
    try {
      ...
      setErrorMsg('Erro ao ler arquivo de áudio: ' + (err?.message || ''));
    }
  };
  ...
  const handleTogglePreviewAudio = () => {
    ...
    if (previewAudioPlaying) {
      ...
      audioPreviewRef.current.play().then(() => {
        ...
        setErrorMsg('Não foi possível tocar o áudio. Verifique se o link ou arquivo é válido.');
      });
    }
  };
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-col gap-2">
                  <div className="flex items-center gap-2">
                    <label className="flex-1 flex items-center justify-center gap-2 px-3 py-2 bg-slate-900 border border-dashed border-slate-700 hover:border-amber-500 rounded-xl text-xs font-semibold text-amber-300 cursor-pointer transition-colors">
                      <Upload className="w-4 h-4 text-amber-400" />
                      <span className="truncate max-w-[200px]">{audioFileName ? audioFileName : 'Enviar Arquivo .MP3'}</span>
                      <input
                        type="file"
