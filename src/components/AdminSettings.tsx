...
export const AdminSettings: React.FC<AdminSettingsProps> = ({
  ...
  // Synchronize internal state whenever settings prop is updated from server / cloud
  useEffect(() => {
    if (settings) {
      ...
      if (settings.proofPhoneNumber) setProofPhoneNumber(settings.proofPhoneNumber);
      if (settings.congregations && settings.congregations.length > 0) {
        setCongregationsList(settings.congregations);
      }
      if (settings.galleryItems && settings.galleryItems.length > 0) {
        setGalleryList(settings.galleryItems);
      }
    }
  }, [settings]);
