...
export const AdminSettings: React.FC<AdminSettingsProps> = ({
  ...
  // Background Hymn / Music (.MP3) State
  const [backgroundMusicUrl, setBackgroundMusicUrl] = useState<string>(settings.backgroundMusicUrl || '');
  const [backgroundMusicTitle, setBackgroundMusicTitle] = useState<string>(settings.backgroundMusicTitle || 'Hino CCB');
  ...
  const fetchSupabaseStatus = async () => {
    ...
    try {
      ...
      console.warn('Could not fetch Supabase status:', e);
    } finally {
  ...
  const handleSyncToSupabase = async () => {
    if (!window.confirm('Deseja sincronizar todos os participantes e cadastros com a tabela do Supabase agora?')) {
      return;
    }
    ...
    try {
      ...
      if (res.ok && data.success) {
        setSuccessMsg(`🚀 ${data.message || 'Sincronização com o Supabase concluída!'}`);
        fetchSupabaseStatus();
      } else {
        setErrorMsg(data.error || 'Erro ao sincronizar com o Supabase.');
      }
    } catch (err: any) {
  ...
  // Local JSON Export (Guaranteed download to PC, Smartphone or Tablet)
  const handleExportLocalJson = async () => {
    setIsExportingJson(true);
    setErrorMsg(null);
    try {
      let backupPayload: FullBackupData | null = null;

      // 1. Try server backup endpoint
      try {
        ...
        console.warn('Direct server export failed, using client data fallback:', e);
      }

      // 2. Client-side fallback if server endpoint had network/offline issue
      if (!backupPayload || !Array.isArray(backupPayload.participants)) {
        try {
          ...
          backupPayload = {
            ...
            settings,
            participants: Array.isArray(pData) ? pData : []
          };
        } catch (errFallback) {
          backupPayload = {
            version: 1,
      ...
      const filename = `backup_gincana_somos_joias_preciosas_${fileDate}.json`;
      downloadLocalJsonBackup(backupPayload, filename);

      setSuccessMsg('✅ Arquivo de backup .JSON gerado e exportado com sucesso para seu dispositivo!');
      setTimeout(() => setSuccessMsg(null), 6000);
    } catch (err: any) {
      console.error('Export JSON error:', err);
      setErrorMsg('Erro ao baixar arquivo JSON: ' + (err?.message || 'Falha ao gerar arquivo.'));
  ...
  // Local JSON Import File (Overwrites server database and local state)
  const handleImportLocalJsonFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;

    if (!window.confirm('Deseja substituir a base de dados atual pelo arquivo JSON selecionado? Todas as configurações e participantes serão sobrescritos com as informações deste arquivo.')) {
      e.target.value = '';
      return;
    }
    ...
    try {
      ...
      // Normalize backup payload
      let payloadToSend: any = {};
      if (Array.isArray(backupData)) {
        payloadToSend = { participants: backupData };
      } else if (backupData && typeof backupData === 'object') {
        payloadToSend = backupData;
      }
      ...
      if (res.ok && responseData.success !== false) {
        ...
        if (responseData.settings) {
          try {
            localStorage.setItem('ccb_gincana_settings', JSON.stringify(responseData.settings));
          } catch (e) {}
        }
        if (payloadToSend.participants || responseData.participants) {
          try {
            localStorage.setItem('ccb_gincana_local_participants', JSON.stringify(payloadToSend.participants || responseData.participants));
          } catch (e) {}
        }

        const countText = responseData.participantsCount !== undefined 
          ? ` (${responseData.participantsCount} participante(s))`
          : '';
        setSuccessMsg(`🎉 Base de dados restaurada e sobrescrita com sucesso a partir do arquivo JSON${countText}! Recarregando...`);

        setTimeout(() => {
          window.location.reload();
        }, 1200);
      } else {
        const errorDetail = responseData.error || responseData.message || 'Falha ao salvar os dados no servidor.';
        setErrorMsg(`Erro ao importar o arquivo JSON no servidor: ${errorDetail}`);
      }
    } catch (err: any) {
      console.error('Import error:', err);
      setErrorMsg('Erro ao processar arquivo de backup: ' + (err?.message || 'Arquivo corrompido ou formato não suportado.'));
    } finally {
  ...
  // Upload Local Audio (.MP3, .WAV, .M4A, .OGG)
  const handleAudioFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;

    if (!file.type.startsWith('audio/') && !file.name.match(/\.(mp3|m4a|wav|ogg|aac)$/i)) {
      setErrorMsg('Por favor, selecione um arquivo de áudio válido (.mp3, .m4a, .wav, .ogg).');
      return;
    }

    if (file.size > 50 * 1024 * 1024) {
      setErrorMsg('O arquivo de áudio tem mais de 50MB. Para arquivos muito pesados, cole o link do Google Drive no campo ao lado.');
      return;
    }
    ...
    try {
      ...
      try {
        ...
        if (res.ok) {
          const data = await res.json();
          if (data.url) {
            audioUrlToSet = data.url;
          }
        }
      } catch (uploadErr) {
        console.warn('Fallback: direct local audio used', uploadErr);
      }

      setBackgroundMusicUrl(audioUrlToSet);
      const autoName = file.name.replace(/\.[^/.]+$/, '').replace(/[-_]/g, ' ');
      if (!backgroundMusicTitle || backgroundMusicTitle === 'Hino CCB') {
        setBackgroundMusicTitle(autoName.charAt(0).toUpperCase() + autoName.slice(1));
      }
      setBackgroundMusicEnabled(true);
      setSuccessMsg('Áudio do hino carregado com sucesso! Clique em "Salvar Configurações" para confirmar.');
    } catch (err: any) {
      setErrorMsg('Erro ao processar arquivo de áudio: ' + (err?.message || ''));
    } finally {
  ...
  const handleTogglePreviewAudio = () => {
    ...
    if (previewAudioPlaying) {
      ...
      audioPreviewRef.current.play().then(() => {
        ...
        setErrorMsg('Não foi possível tocar o áudio. Verifique se o link ou arquivo é válido e público.');
      });
    }
  };

  const handleRemoveAudio = () => {
    if (previewAudioPlaying && audioPreviewRef.current) {
      audioPreviewRef.current.pause();
      setPreviewAudioPlaying(false);
    }
    ...
    setSuccessMsg('Hino em áudio removido.');
  };
    ...
    <div className="max-w-4xl mx-auto space-y-6 pb-12">
      <div className="bg-slate-900 border border-amber-500/30 rounded-3xl p-6 sm:p-8 shadow-2xl space-y-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          {/* Section: Hino em .MP3 / Música de Fundo (Tela Inicial) */}
          <div className="bg-slate-950 border border-slate-800 p-5 rounded-2xl space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-800 pb-3">
              <div>
                <h3 className="text-xs font-bold text-amber-300 uppercase tracking-wider flex items-center gap-2">
                  <Music className="w-4 h-4 text-amber-400" /> Hino em .MP3 / Música de Fundo (Tela Inicial)
                </h3>
                <p className="text-[11px] text-slate-400 mt-0.5">
                  A música oficial é carregada diretamente da pasta local <code className="text-amber-300 bg-slate-900 px-1.5 py-0.5 rounded">uploads/audio/</code>.
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  onChange={(e) => setBackgroundMusicTitle(e.target.value)}
                  placeholder="Ex: Se Vós Baterdes Ele Vos Abre"
                ...
                />
                <p className="text-[11px] text-slate-400">
                  Exibido na barra e no tocador de áudio na tela inicial do evento.
                </p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="p-3 bg-slate-900 border border-slate-700/80 rounded-xl flex items-center justify-between gap-3">
                  <div className="flex items-center gap-2.5 overflow-hidden">
                    ...
                    <div className="overflow-hidden">
                      <p className="text-xs font-bold text-white truncate">
                        Se_vos_baterdes_Ele_vos_abre.mp3
                      </p>
                      <p className="text-[10px] text-emerald-400 font-medium">
                        ✓ Vinculado na pasta uploads/audio/
                      </p>
                    </div>
                  </div>
                  <span className="px-2 py-0.5 rounded text-[10px] font-mono bg-slate-950 text-amber-300 border border-slate-800 shrink-0">
                    MP3
                  </span>
                </div>
                <p className="text-[11px] text-slate-400">
                  Reprodução direta otimizada para todos os participantes do portal.
                </p>
              </div>
            </div>

            {/* Test Player & Preview */}
            <div className="pt-2 border-t border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-slate-900/80 p-4 rounded-xl border border-slate-800">
              <div className="flex items-center gap-3">
                <button
                  type="button"
                  onClick={handleTogglePreviewAudio}
                  className="w-10 h-10 rounded-full bg-amber-500 hover:bg-amber-400 text-slate-950 flex items-center justify-center shadow-lg shadow-amber-500/20 cursor-pointer transition-transform active:scale-95 shrink-0"
                  title={previewAudioPlaying ? 'Pausar Teste' : 'Ouvir Teste'}
                >
                  {previewAudioPlaying ? <Pause className="w-5 h-5 fill-slate-950" /> : <Play className="w-5 h-5 fill-slate-950 ml-0.5" />}
                </button>
                ...
                <div className="overflow-hidden">
                  ...
                  <span className="text-[10px] text-amber-400/90 flex items-center gap-1 mt-0.5">
                    ...
                    {previewAudioPlaying ? 'Reproduzindo teste do áudio...' : 'Clique no play para testar a reprodução'}
                  </span>
                </div>
                ...
                <audio
                  ref={audioPreviewRef}
                  src={formatAudioUrl(backgroundMusicUrl || '/uploads/audio/Se_vos_baterdes_Ele_vos_abre.mp3')}
                  onEnded={() => setPreviewAudioPlaying(false)}
                  onError={() => {
                    setPreviewAudioPlaying(false);
                    console.warn('Erro ao carregar preview do áudio.');
                  }}
                  className="hidden"
                />
              </div>
              ...
              <div className="text-[11px] text-slate-400 flex items-center gap-2">
                <span className="px-2 py-0.5 rounded bg-slate-950 border border-slate-800 text-emerald-400 font-medium text-[10px]">
                  Áudio Pronto
                </span>
              </div>
            </div>
          ...
          {/* Section: Supabase Cloud Database & Storage (PostgreSQL) */}
          <div className="bg-slate-950/90 border border-emerald-500/40 p-5 rounded-2xl space-y-4 shadow-xl">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-800 pb-3">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-sm font-extrabold text-white">
                    Banco de Dados Definitivo & Armazenamento (Supabase)
                  </h3>
                </div>
                <p className="text-xs text-slate-400 mt-1">
                  Gerencia os participantes, configurações e comprovantes via PostgreSQL em nuvem de alta velocidade.
                </p>
              </div>
              ...
              <div className="flex items-center gap-2">
                {loadingSupabaseStatus ? (
                  <span className="px-3 py-1 bg-slate-900 border border-slate-700 text-slate-400 text-xs rounded-full flex items-center gap-1.5">
                    <RefreshCw className="w-3.5 h-3.5 animate-spin" /> Verificando...
                  </span>
                ) : supabaseStatus?.connected ? (
                  <span className="px-3 py-1 bg-emerald-500/20 border border-emerald-500/50 text-emerald-300 text-xs font-bold rounded-full flex items-center gap-1.5">
                    <CheckCircle2 className="w-4 h-4 text-emerald-400" /> Supabase Conectado ({supabaseStatus.participantsCount} registros)
                  </span>
                ) : (
                  <span className="px-3 py-1 bg-amber-500/20 border border-amber-500/40 text-amber-300 text-xs font-semibold rounded-full flex items-center gap-1.5">
                    <AlertCircle className="w-4 h-4 text-amber-400" /> Configuração do Supabase
                  </span>
                )}
                <button
                  type="button"
            ...
            {/* Supabase details & Actions */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-1">
              <div className="p-3.5 bg-slate-900/90 rounded-xl border border-slate-800 space-y-2">
                <p className="text-xs font-bold text-slate-200 flex items-center gap-1.5">
                  <Server className="w-4 h-4 text-emerald-400" /> 1. Criar Tabelas e Bucket no Supabase
                </p>
                <p className="text-[11px] text-slate-400 leading-relaxed">
                  No painel do Supabase, clique em <strong>SQL Editor</strong> &gt; <strong>New query</strong>, cole o script oficial e clique em <strong>Run</strong>.
                </p>
                <div className="flex items-center gap-2 pt-1">
                  <button
                    type="button"
                    onClick={handleCopySqlScript}
                    className="px-3.5 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs flex items-center gap-1.5 transition-all shadow-md active:scale-95 cursor-pointer"
                  >
                    {copiedSql ? <Check className="w-4 h-4" /> : <Copy className="w-4 h-4" />}
                    {copiedSql ? 'Copiado para Área de Transferência!' : 'Copiar Script SQL Oficial'}
                  </button>
                  <button
                    type="button"
                  ...
                  >
                    {showSqlScript ? 'Ocultar SQL' : 'Visualizar SQL'}
                  </button>
                </div>
              </div>
              ...
              <div className="p-3.5 bg-slate-900/90 rounded-xl border border-slate-800 space-y-2">
                <p className="text-xs font-bold text-slate-200 flex items-center gap-1.5">
                  <RefreshCw className="w-4 h-4 text-amber-400" /> 2. Sincronizar Participantes Existentes
                </p>
                <p className="text-[11px] text-slate-400 leading-relaxed">
                  Envie todos os inscritos atuais para o banco de dados Supabase com 1 clique para garantir permanência definitiva.
                </p>
                <div className="pt-1">
                  <button
                    type="button"
                    ...
                    className="px-4 py-2 rounded-xl bg-amber-500 hover:bg-amber-400 disabled:opacity-50 text-slate-950 font-bold text-xs flex items-center gap-1.5 transition-all shadow-md active:scale-95 cursor-pointer"
                  >
                    <RefreshCw className={`w-4 h-4 ${isSyncingSupabase ? 'animate-spin' : ''}`} />
                    {isSyncingSupabase ? 'Sincronizando...' : 'Sincronizar com o Supabase Agora'}
                  </button>
                </div>
              </div>
            </div>
            ...
            {showSqlScript && (
              <div className="p-3.5 bg-slate-900 rounded-xl border border-slate-800 space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-amber-400 font-mono">Script SQL (schema.sql):</span>
                  <button
                    type="button"
                    onClick={handleCopySqlScript}
                ...
                <pre className="p-3 bg-slate-950 text-slate-300 rounded-lg text-[11px] font-mono overflow-x-auto max-h-48 border border-slate-800">
            ...
            <div className="p-3 bg-slate-900/60 rounded-xl border border-slate-800/80 text-[11px] text-slate-400 space-y-1">
              <p className="font-bold text-slate-300">Variáveis necessárias na Vercel (Settings &gt; Environment Variables):</p>
              <ul className="list-disc pl-4 space-y-0.5 font-mono text-[10px] text-amber-300/90">
                <li>SUPABASE_URL=https://[seu-projeto].supabase.co</li>
                <li>SUPABASE_KEY=[sua-chave-anon-ou-service-role]</li>
              </ul>
            </div>
          </div>
          ...
          <div className="bg-slate-950/80 border border-slate-800 p-5 rounded-2xl space-y-3">
            ...
            <div className="flex flex-wrap gap-3 pt-1">
              <button
                id="btn-download-local-json"
                type="button"
                onClick={handleExportLocalJson}
                disabled={isExportingJson}
                className="px-4 py-2.5 rounded-xl bg-amber-500 hover:bg-amber-400 disabled:opacity-50 text-slate-950 font-bold text-xs flex items-center gap-2 cursor-pointer transition-all shadow-md shadow-amber-500/10 active:scale-95"
              >
                <Download className={`w-4 h-4 text-slate-950 ${isExportingJson ? 'animate-bounce' : ''}`} />
                {isExportingJson ? 'Gerando Arquivo JSON...' : 'Baixar Arquivo JSON Local'}
              </button>

              <label className={`px-4 py-2.5 rounded-xl bg-slate-900 hover:bg-slate-800 border border-slate-700 text-slate-200 font-bold text-xs flex items-center gap-2 cursor-pointer transition-colors ${isImportingJson ? 'opacity-60 pointer-events-none' : ''}`}>
                <Upload className={`w-4 h-4 text-emerald-400 ${isImportingJson ? 'animate-spin' : ''}`} />
                {isImportingJson ? 'Restaurando Base de Dados...' : 'Carregar Arquivo JSON Local'}
                <input
                  id="input-import-local-json"
                  type="file"
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <label className="text-[11px] font-bold text-slate-300 block flex items-center gap-1.5">
                    <Link2 className="w-3.5 h-3.5 text-amber-400" /> Ou Cadastrar por Link em Nuvem (Google Drive, ImgBB, YouTube)
                  </label>
                  <span className="text-[10px] text-amber-400 font-semibold bg-amber-500/10 px-2 py-0.5 rounded-full border border-amber-500/20">
                    Recomendado para dezenas de fotos/vídeos
                  </span>
                </div>
                <div className="flex gap-2">
                  <input
                    ...
                    placeholder="Cole o link do Google Drive, ImgBB, YouTube, Postimages ou link direto (.jpg, .png, .mp4)..."
                ...
                <div className="mt-2 p-3 bg-slate-950/80 border border-slate-800/80 rounded-xl space-y-2 text-[11px] text-slate-300">
                  <p className="font-bold text-amber-400 flex items-center gap-1.5">
                    💡 Como hospedar e pegar o link para adicionar aqui:
                  </p>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-2 pt-1 text-[11px]">
                    <div className="p-2.5 bg-slate-900/90 rounded-lg border border-slate-800">
                      <p className="font-bold text-white mb-1 flex items-center gap-1">📁 Google Drive</p>
                      <p className="text-slate-400 text-[10px] leading-relaxed">
                        1. Suba a foto no seu Google Drive.<br />
                        2. Botão direito na foto &gt; <strong>Compartilhar</strong>.<br />
                        3. Altere o acesso para <strong>"Qualquer pessoa com o link"</strong>.<br />
                        4. Copie o link e cole aqui no campo acima! (O sistema converte automaticamente).
                      </p>
                    </div>
                    <div className="p-2.5 bg-slate-900/90 rounded-lg border border-slate-800">
                      <p className="font-bold text-white mb-1 flex items-center gap-1">🖼️ ImgBB / Postimages (Grátis)</p>
                      <p className="text-slate-400 text-[10px] leading-relaxed">
                        1. Acesse <strong>imgbb.com</strong> ou <strong>postimages.org</strong> (não precisa cadastro).<br />
                        2. Envie suas fotos.<br />
                        3. Copie o <strong>"Link direto da imagem"</strong> (.jpg ou .png).<br />
                        4. Cole aqui. Carregamento ultra rápido!
                      </p>
                    </div>
                    <div className="p-2.5 bg-slate-900/90 rounded-lg border border-slate-800">
                      <p className="font-bold text-white mb-1 flex items-center gap-1">🎥 Vídeos (YouTube / Vimeo)</p>
                      <p className="text-slate-400 text-[10px] leading-relaxed">
                        1. Envie seu vídeo no <strong>YouTube</strong> (pode ser "Não listado" ou público).<br />
                        2. Copie o link do vídeo ou Shorts.<br />
                        3. Cole aqui selecionando o tipo "Vídeo Embed". O player roda direto no app!
                      </p>
                    </div>
                  </div>
                </div>
