...
export const AdminSettings: React.FC<AdminSettingsProps> = ({
  ...
  const handleImportLocalJsonFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (res.ok) {
        setSuccessMsg('🎉 Base de dados restaurada com sucesso a partir do arquivo JSON local!');
        setTimeout(() => {
          window.location.reload();
        }, 1500);
      } else {
        setErrorMsg('Erro ao importar o arquivo JSON no servidor.');
      }
    } catch (err: any) {
      setErrorMsg('Erro ao processar arquivo de backup: ' + (err?.message || ''));
    } finally {
      e.target.value = '';
    }
  };
