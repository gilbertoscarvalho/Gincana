...
import { fetchParticipantsFromSupabaseDirect, saveParticipantToSupabaseDirect } from '../lib/supabase';
import {
  isPdfDocument,
...
export const ProofLookup: React.FC<ProofLookupProps> = ({ onUpdated, pixKey, whatsappGroupUrl, congregations }) => {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Fallback to searching Supabase participants if server returned nothing or was offline
      if (matches.length === 0) {
        try {
          const supabaseList = await fetchParticipantsFromSupabaseDirect();
