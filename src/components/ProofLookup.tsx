...
import { searchParticipantsInSupabaseDirect, saveParticipantToSupabaseDirect, uploadFileToSupabaseStorage } from '../lib/supabase';
import {
  isPdfDocument,
...
export const ProofLookup: React.FC<ProofLookupProps> = ({ onUpdated, pixKey, whatsappGroupUrl, congregations }) => {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Fallback to targeted search on Supabase (never downloads whole table!)
      if (matches.length === 0) {
        try {
          const directMatches = await searchParticipantsInSupabaseDirect(
            cleanName,
            finalCong !== 'todas' ? finalCong : undefined,
  ...
  // Compress image to small JPEG Blob (< 150 KB)
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 15 * 1024 * 1024) {
      setSearchError('O arquivo é muito grande. Tamanho máximo permitido: 15MB.');
      return;
    }
    ...
    try {
      ...
      if (!publicUrl) {
        try {
          ...
          if (serverRes.ok) {
            ...
            if (serverData.url) publicUrl = serverData.url;
          }
        } catch {}
      }

      if (!publicUrl) {
        publicUrl = URL.createObjectURL(uploadPayload);
      }
      ...
      setSearchError('Erro ao processar arquivo. Tente novamente.');
    }
  };
  ...
  const handleSaveProof = async () => {
    ...
    try {
      ...
      // Ensure persistence to Supabase directly
      saveParticipantToSupabaseDirect(updatedParticipant).catch(() => {});
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm font-semibold text-white truncate max-w-[220px] sm:max-w-xs">
                    {selectedParticipant.proofFileName || (isPdfDocument(selectedParticipant.proofFileType, selectedParticipant.proofFileName, selectedParticipant.proofUrl) ? 'comprovante.pdf' : 'comprovante.png')}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <button
                  ...
                  onClick={() =>
                    openProofInNewTab(
                      selectedParticipant.proofUrl,
                      selectedParticipant.proofFileName || 'comprovante.pdf'
                    )
                  }
                ...
                >
                  <ExternalLink className="w-3.5 h-3.5" /> Visualizar Arquivo
                </button>
                <button
                  type="button"
                  onClick={() =>
                  ...
                  className="px-3 py-1.5 bg-amber-500/20 hover:bg-amber-500/30 text-amber-300 border border-amber-500/40 rounded-xl font-bold text-xs cursor-pointer transition-colors"
                >
                  Baixar
                </button>
              </div>
