...
import { Search, FileCheck, CheckCircle2, AlertCircle, Upload, FileText, User, Church, Calendar, ArrowRight, X, QrCode, Copy, Check, Phone, MessageCircle } from 'lucide-react';
...
export const ProofLookup: React.FC<ProofLookupProps> = ({ onUpdated, proofPhoneNumber }) => {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.error || 'Cadastro não localizado com estes dados.');
      }
  ...
  const handleSaveProof = async () => {
    ...
    try {
      const res = await fetch(`/api/participants/${selectedParticipant.id}/proof`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  return (
    <div className="max-w-3xl mx-auto my-6 px-4">
      ...
      <div className="bg-gradient-to-r from-amber-500/10 via-slate-950 to-emerald-500/10 border border-amber-500/40 rounded-3xl p-5 mb-6 space-y-3 shadow-xl">
        ...
        {proofPhoneNumber && (
          <div className="pt-2.5 border-t border-amber-500/20 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-slate-900/60 p-3 rounded-2xl border border-slate-800">
            ...
            <div className="flex items-center gap-2">
              ...
              <a
                ...
                className="px-3.5 py-1.5 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold text-xs flex items-center gap-1.5 transition-colors cursor-pointer shadow-md"
              >
