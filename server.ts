...
async function fetchParticipantsFromSupabase(): Promise<Participant[] | null> {
  ...
  try {
    ...
    if (Array.isArray(data)) {
      ...
      if (sanitized.length === 0 && diskList.length > 0) {
        saveAllParticipantsToSupabase(diskList).catch(() => {});
        cachedParticipants = diskList;
        return diskList;
      }

      // If both have items or Supabase has items, merge them safely
      const merged = mergeParticipants(diskList, sanitized);
      cachedParticipants = merged;
      saveParticipantsToDisk(merged);

      if (merged.length > sanitized.length) {
        saveAllParticipantsToSupabase(merged).catch(() => {});
      }
