...
async function initServerStorage(): Promise<void> {
  ...
  // 3. Check Vercel Blob cloud backup storage and merge without overwriting newer local data
  try {
      ...
      if (blobs && blobs.length > 0) {
        ...
        if (res.ok) {
          ...
          if (cloudData.participants && Array.isArray(cloudData.participants) && cloudData.participants.length > 0) {
            // Merge cloud participants with disk participants to preserve any new entries
          ...
          console.log(`[Storage] Base de dados sincronizada! (${cachedParticipants?.length || 0} participantes)`);
        }
      } else {
...
function loadSettings(): EventSettings {
  ...
  try {
    if (fs.existsSync(SETTINGS_FILE)) {
      ...
      if (loaded.backgroundMusicUrl && (loaded.backgroundMusicUrl.includes('peaceful-piano') || loaded.backgroundMusicUrl.includes('pixabay.com'))) {
        loaded.backgroundMusicUrl = '';
        loaded.backgroundMusicTitle = '';
        loaded.backgroundMusicEnabled = false;
      }
      cachedSettings = {
        ...DEFAULT_SETTINGS,
        ...loaded,
