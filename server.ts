    ...
    if (blobs && blobs.length > 0) {
      ...
      if (res.ok) {
        ...
        if (cloudData.participants && Array.isArray(cloudData.participants)) {
          ...
          const merged = mergeParticipants(localDiskParticipants, sanitizedCloud);
          cachedParticipants = sanitizeParticipants(merged);
          try {
            if (!fs.existsSync(DATA_DIR)) fs.mkdirSync(DATA_DIR, { recursive: true });
...
// Default empty seed list for participants (clean database ready for real registrations)
...
function loadParticipants(): Participant[] {
  try {
    if (fs.existsSync(DATA_FILE)) {
      ...
      if (Array.isArray(parsed)) {
        const sanitized = sanitizeParticipants(parsed);
        cachedParticipants = sanitized;
        return sanitized;
      }
    }
  ...
  try {
    ...
    console.warn('Unable to write participants file:', err);
  }
  return cachedParticipants;
}
...
async function saveParticipants(participants: Participant[]): Promise<void> {
  ...
  // Also ensure process.cwd()/data/participants.json is kept in sync if DATA_DIR is /tmp/data
  ...
  syncBackupToVercelBlobInBackground(sanitized, cachedSettings || loadSettings()).catch((err) => {
    console.warn('[Storage] Background sync error:', err);
  });
}
