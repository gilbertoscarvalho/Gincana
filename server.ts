...
const DEFAULT_SUPABASE_PUBLISHABLE_KEY = 'sb_publishable_a_qUOnQnE8Vi9i5y0JsE0w_-lTNbc3C';
const DEFAULT_SUPABASE_JWKS_URL = 'https://ykhzsdnbprosatvpygbb.supabase.co/auth/v1/.well-known/jwks.json';
const DEFAULT_SUPABASE_KEY = DEFAULT_SUPABASE_PUBLISHABLE_KEY;
...
function getSupabaseServerClient(): SupabaseClient | null {
  const url = (
    customServerSupabaseUrl ||
    process.env.SUPABASE_URL ||
    process.env.VITE_SUPABASE_URL ||
    DEFAULT_SUPABASE_URL
  ).trim();

  let key = customServerSupabaseKey ? customServerSupabaseKey.trim() : '';
  if (!key) {
    const candidateKeys = [
    ...
    // Filter out stale keys from the legacy project (e.g. containing t0V20lTkdh)
    const validCandidates = candidateKeys.filter(k => !k.includes('t0V20lTkdh'));
    key = validCandidates[0] || DEFAULT_SUPABASE_KEY;
  }
...
// Convert application Participant model to Supabase table row (providing full compatibility with both camelCase and snake_case PostgreSQL schemas)
...
async function fetchParticipantsFromSupabase(forceFresh = false): Promise<Participant[] | null> {
  ...
  try {
    const { data, error } = await supabase
      .from('participants')
      .select('id, full_name, first_name, congregation, age, food_or_drink, activities, proof_url, proof_file_name, proof_file_type, proof_status, team_id, notes, amount_paid, created_at, updated_at, "fullName", "firstName", "foodOrDrink", "proofUrl", "proofFileName", "proofFileType", "proofStatus", "teamId", "amountPaid", "createdAt", "updatedAt"')
...
async function executeResilientServerSupabaseUpsert(supabase: any, row: Record<string, any>): Promise<boolean> {
  ...
  // Attempt 4: Base snake_case without newer columns (preserved in activities JSONB)
  ...
  // Attempt 5: Base camelCase without newer columns
  ...
  console.warn('[Supabase Server] Notice upserting participant with all schema fallbacks:', baseCamelErr?.message || fullErr?.message);
  return false;
}
...
const supabaseConfigHandler = async (req: Request, res: Response) => {
  ...
  const activeUrl = (customServerSupabaseUrl || process.env.SUPABASE_URL || process.env.VITE_SUPABASE_URL || DEFAULT_SUPABASE_URL).trim();
  const rawKey = (
    customServerSupabaseKey ||
    process.env.SUPABASE_PUBLISHABLE_KEY ||
    process.env.VITE_SUPABASE_PUBLISHABLE_KEY ||
    process.env.VITE_SUPABASE_ANON_KEY ||
    process.env.SUPABASE_ANON_KEY ||
  ...
  const activeKey = rawKey.includes('t0V20lTkdh') ? DEFAULT_SUPABASE_PUBLISHABLE_KEY : rawKey;
...
const supabaseStatusHandler = async (req: Request, res: Response) => {
  const supabase = getSupabaseServerClient();
  const url = (customServerSupabaseUrl || process.env.SUPABASE_URL || process.env.VITE_SUPABASE_URL || DEFAULT_SUPABASE_URL).trim();
