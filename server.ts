...
async function initServerStorage(): Promise<void> {
  ...
  let localParticipantsLoaded = false;
  let localSettingsTime = 0;
  let localParticipantsTime = 0;

  // First step: read disk if available
  try {
    if (fs.existsSync(SETTINGS_FILE)) {
      const stat = fs.statSync(SETTINGS_FILE);
      localSettingsTime = stat.mtimeMs;
  ...
  try {
    if (fs.existsSync(DATA_FILE)) {
      ...
      localParticipantsTime = stat.mtimeMs;
      const data = fs.readFileSync(DATA_FILE, 'utf-8');
      const loaded = JSON.parse(data);
      if (Array.isArray(loaded)) {
        cachedParticipants = loaded;
        localParticipantsLoaded = true;
      }
    }
  } catch (err) {
  ...
  try {
    ...
    if (token) {
      ...
      if (blobs && blobs.length > 0) {
        ...
        const cloudUploadTime = new Date(backupBlob.uploadedAt).getTime();

        // Only fetch and apply cloud data if local data is missing OR cloud data is strictly newer than local files
        const shouldLoadCloud = !localSettingsLoaded || !localParticipantsLoaded || (cloudUploadTime > Math.max(localSettingsTime, localParticipantsTime) + 10000);

        if (shouldLoadCloud) {
          const res = await fetch(backupBlob.url, { cache: 'no-store' });
          if (res.ok) {
            const cloudData = await res.json();
            if (cloudData.settings && typeof cloudData.settings === 'object' && !localSettingsLoaded) {
              cachedSettings = { ...DEFAULT_SETTINGS, ...cloudData.settings };
              try {
                fs.writeFileSync(SETTINGS_FILE, JSON.stringify(cachedSettings, null, 2));
              } catch (e) {}
            }
            if (cloudData.participants && Array.isArray(cloudData.participants) && !localParticipantsLoaded) {
          ...
          // Local disk is newer or equal: push local data to Vercel Blob to keep it updated!
          console.log('[Storage] Disco local contém os dados mais recentes. Sincronizando com a nuvem...');
          if (cachedParticipants && cachedSettings) {
            syncBackupToVercelBlobInBackground(cachedParticipants, cachedSettings);
          }
        }
      } else {
        // No cloud blob exists yet: create initial backup with local data
        if (cachedParticipants && cachedSettings) {
          syncBackupToVercelBlobInBackground(cachedParticipants, cachedSettings);
        }
      }
