...
import type { Participant, DashboardStats, EventSettings } from './src/types';
...
async function syncFromVercelBlobInBackground(localDiskParticipants: Participant[]): Promise<void> {
  try {
    ...
    // If local disk file exists and was loaded, prioritize it and update the cloud backup
    const diskFileExists = fs.existsSync(DATA_FILE);
    if (diskFileExists && cachedParticipants !== null) {
    ...
    if (blobs && blobs.length > 0) {
      ...
      if (res.ok) {
        ...
        if (cloudData.participants && Array.isArray(cloudData.participants) && !diskFileExists) {
...
function loadParticipantsFromDisk(): Participant[] {
  try {
    ...
    console.warn('[Disk Storage] Error reading local participants file:', err);
  }
...
async function mutateAndCommitParticipants(
...
): Promise<{ participants: Participant[]; fromGitHub: boolean }> {
  const config = getGitHubConfig();

  if (!config.token) {
    const current = loadParticipantsFromDisk();
    const updated = sanitizeParticipants(mutationFn(current));
    saveParticipantsToDisk(updated);
    cachedParticipants = updated;
    return { participants: updated, fromGitHub: false };
  }
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      // 1. Fetch latest SHA & file content
      const { participants: current, sha, filePath } = await fetchParticipantsFromGitHub();

      // 2. Apply mutation
      const updated = sanitizeParticipants(mutationFn(current));

      // 3. Prepare Base64 payload
      ...
      if (res.status === 409) {
        // Concurrency conflict: another client committed simultaneously!
        console.warn(`[GitHub Storage] Concurrency conflict (409) on attempt ${attempt}/${maxAttempts}. Refreshing SHA and retrying...`);
        await new Promise((resolve) => setTimeout(resolve, 250 * attempt + Math.random() * 100));
        continue;
      }
      ...
      // 5. Update local disk and in-memory cache
      saveParticipantsToDisk(updated);
      cachedParticipants = updated;

      console.log(`[GitHub Storage] Successfully committed ${updated.length} participants to ${config.owner}/${config.repo} (${filePath}).`);
      return { participants: updated, fromGitHub: true };
    } catch (err: any) {
      lastErr = err;
      console.warn(`[GitHub Storage] Mutation attempt ${attempt} error:`, err?.message || err);
  ...
  throw new Error(`Erro ao persistir no repositório GitHub: ${lastErr?.message || 'Falha de comunicação.'}`);
}
...
const getParticipantsHandler = async (req: Request, res: Response) => {
  try {
    const { participants } = await fetchParticipantsFromGitHub();
