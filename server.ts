...
import { Participant, DashboardStats, EventSettings } from './src/types';
...
async function startServer() {
  ...
  if (process.env.NODE_ENV !== 'production') {
    ...
    app.get('*', (req: Request, res: Response) => {
      ...
      // Safe HTML fallback shell if index.html is not packaged on disk
      const fallbackHtml = `<!doctype html>
<html lang="pt-BR">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>Somos Jóias Preciosas • Portal da Gincana & Tocata</title>
