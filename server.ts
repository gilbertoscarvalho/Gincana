...
const audioProxyHandler = async (req: Request, res: Response) => {
  try {
    ...
    let fileId = driveId;
    if (!fileId && rawUrl) {
      const gDriveMatch = rawUrl.match(/drive\.google\.com\/(?:file\/d\/|open\?id=|uc\?(?:export=download&)?id=)([a-zA-Z0-9_-]+)/);
    ...
    if (!targetUrl) {
      return res.status(400).json({ error: 'URL ou ID de áudio não informado.' });
    }
    ...
    if (!upstreamRes.ok && upstreamRes.status !== 206 && fileId) {
      // Secondary attempt with uc?export=download
      const altUrl = `https://drive.google.com/uc?export=download&id=${fileId}`;
      upstreamRes = await fetch(altUrl, {
        method: 'GET',
        headers: forwardHeaders,
        redirect: 'follow'
      });
    ...
    if (!upstreamRes.ok && upstreamRes.status !== 206) {
      return res.status(upstreamRes.status).json({
        error: 'Não foi possível carregar o arquivo do Google Drive. Verifique se o link possui permissão "Qualquer pessoa com o link pode ler".'
      });
    }
