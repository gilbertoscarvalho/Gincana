...
// Convert application Participant model to Supabase table row (using exact PostgreSQL table schema + JSONB backup for amount_paid and notes)
...
async function upsertParticipantInSupabase(participant: Participant): Promise<boolean> {
  ...
  try {
    ...
    // Fallback: if amount_paid or notes do not exist as top-level columns in PostgreSQL yet, upsert without them.
    // row.activities already contains amount_paid, amountPaid, and notes, so the data is 100% saved in Supabase JSONB!
    if (error && (error.message?.includes('amount_paid') || error.message?.includes('notes') || error.code === 'PGRST204')) {
      console.warn('[Supabase Server] Retrying upsert with JSONB fallback:', error.message);
